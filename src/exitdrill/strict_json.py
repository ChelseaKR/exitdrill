...
def _object_from_pairs(pairs: list[tuple[str, object]]) -> dict[str, object]:
    ...
    for key, value in pairs:
        if key in result:
            raise StrictJsonError(f"duplicate object key is not permitted: {key}")
        result[key] = value
    return result


def validate_json_value(value: object, *, max_depth: int = _MAX_JSON_DEPTH) -> None:
    """Reject non-finite floats and container nesting beyond ``max_depth``."""
