...
| Successful index verification is presented as full artifact validation | CLI output names binding-and-schema-header scope only and performs no result aggregation or structural evaluation | callers may omit the scope and overstate byte consistency as semantic validity |
